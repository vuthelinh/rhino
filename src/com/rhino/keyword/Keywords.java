...
public class Keywords {
	...
	public static void scrollBrowser(String object, String data) {
		/*
		 * Scroll the document window to the horizontal position x-axis and
		 * vertical y-axis
		 */
		String Data[] = data.split(",");
		int x = Integer.parseInt(Data[0]);
		int y = Integer.parseInt(Data[1]);
		APP_LOGS.debug("[info] Executing: |Scroll browser to the horizontal position " + Data[0] + " and vertical "
				+ Data[1] + " | ");
	...
	public static String addSelection(String object, String data) {
		...
		try {
			...
			for (int K = 0; K < Data.length; K++) {
				int j = 0;
				for (int i = 0; i < list.size(); i++)// For loop to select a
														// data from listbox
				{
					j++;
	...
	public static String chooseCancelOnNextConfirmation(String object, String data) {
		...
		try {
			...
			myAlert.dismiss(); // cancel the alert - equivalent of pressing
								// CANCEL
			return Const.PASS;
	...
	public static String chooseOkOnNextConfirmation(String object, String data) {
		...
		try {
			Alert myAlert = _driver.switchTo().alert(); // Switch to alert
														// pop-up
	...
	public static String open(String object, String data) {
		...
		try {
			...
			int No = Integer.parseInt(CONFIG.getProperty("selenium.browser"));
			_driver = openBrowser(No);
	...
	private static WebDriver openBrowser(int browserType) {
	...
	private static int storeXpathCount(String object) {
		...
		try {
			...
			APP_LOGS.debug(
					"[>>>> ERROR <<<] Element " + OBJECT.getProperty(object) + " not found \n " + e.getMessage());
		}
	...
	public static String clickAt(String object, String data) {
		...
		/*
		 * data - specifies the x,y position (i.e. - 10,20) of the mouse event
	...
	public static String selectByVisibleText(String object, String data) {
		...
		try {
			...
			// For loop to split and take single data from excel sheet
			int j = 0;
			for (int i = 0; i < list.size(); i++)// For loop to select a data
													// from listbox
			{
	...
	public static String iMouseHover(String object, String data) {
		...
		try {
			...
			APP_LOGS.debug(
					"[>>>> ERROR <<<] Element " + OBJECT.getProperty(object) + " not on screen !" + e.getMessage());
			return Const.FAIL + ": Element " + OBJECT.getProperty(object) + " not on screen !";
		}
	}
	...
	public static String verifyFileExist(String object, String data) {
		...
		try {
			if (templateFile.getCellData("Sheet1", 0, 1).equals("Depth")
					&& templateFile.getCellData("Sheet1", 1, 1).equals("Pore pressuree")) {
				return Const.PASS;
			}
